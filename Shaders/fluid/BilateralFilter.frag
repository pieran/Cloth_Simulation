...
uniform vec2 esBlurDir;	//eye-space blur dir
uniform vec2 tsBlurDir;	//texture-space blur dir
uniform float blurScale = 0.148125f;
uniform float blurDepthFalloff = 500.0f;
...
void main(void)	{
	...
	float filterRadius = clamp(sqrt(dot(tsDistance, tsDistance)) * 30.0f, 1.0f, 30.0f);
	filterRadius = 15.0f;//5.0f;
	...
	vec2 pix = vec2(1.0f / 1370.0f, 1.0f / 768.0f);
	...
	gl_FragColor = vec4(vec3(asum), 1.0f);
}