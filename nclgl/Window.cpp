...
void Window::SetWindowSize(int width, int height)
{
	...
	ptDiff.x = (rw.right - rw.left) - rc.right;
	ptDiff.y = (rw.bottom - rw.top) - rc.bottom;

	SetWindowPos(windowHandle, HWND_TOP, rw.left, rw.top, width + ptDiff.x, height + ptDiff.y, SWP_SHOWWINDOW);
...
void Window::CheckMessages(MSG &msg)	{
	switch (msg.message)	{				// Is There A Message Waiting?
		...
		case (WM_MOUSEMOVE) : {
			mousePosition.x = (float)LOWORD(msg.lParam);
			mousePosition.y = (float)HIWORD(msg.lParam);
		}break;
		case (WM_INPUT): {
			UINT dwSize;
