...
void NCLDebug::DrawMatrix(const Matrix4& mtx, float scale)
{
	Vector3 position = mtx.GetPositionVector();
	DrawHairLine(position, position + Vector3(mtx.values[0], mtx.values[1], mtx.values[2]) * scale, Vector4(1.0f, 0.0f, 0.0f, 1.0f));
	DrawHairLine(position, position + Vector3(mtx.values[4], mtx.values[5], mtx.values[6]) * scale, Vector4(0.0f, 1.0f, 0.0f, 1.0f));
	DrawHairLine(position, position + Vector3(mtx.values[8], mtx.values[9], mtx.values[10]) * scale, Vector4(0.0f, 0.0f, 1.0f, 1.0f));
}

void NCLDebug::DrawMatrix(const Matrix3& mtx, const Vector3& position, float scale)
{
	DrawHairLine(position, position + mtx.GetCol(0) * scale, Vector4(1.0f, 0.0f, 0.0f, 1.0f));
	DrawHairLine(position, position + mtx.GetCol(1) * scale, Vector4(0.0f, 1.0f, 0.0f, 1.0f));
	DrawHairLine(position, position + mtx.GetCol(2) * scale, Vector4(0.0f, 0.0f, 1.0f, 1.0f));
}
...
void NCLDebug::DrawDebugLists()
{
	...
	if (m_pShaderHairLines && m_Tris.size() > 0)
	{
	...
	if (m_pShaderHairLines && m_HairLines.size() > 0)
	{
		glUseProgram(m_pShaderHairLines->GetProgram());
		glUniformMatrix4fv(glGetUniformLocation(m_pShaderHairLines->GetProgram(), "projViewMatrix"), 1, GL_FALSE, &m_ProjView.values[0]);
