...
void Scene::RenderScene()
{
	...
	SetCurrentShader(m_DefaultShadowShader);
	m_renderStage = RENDER_TYPE_DARK;
	DrawNodes(false);

	//Render the Shadow Volumes to the Stencil Buffer
	SetCurrentShader(m_ShadowVolumeShader);
	m_renderStage = RENDER_TYPE_SHADOW_EXTRUDE;
	glDepthFunc(GL_LESS);
	...
	SetCurrentShader(m_DefaultLightShader);
	m_renderStage = RENDER_TYPE_LIGHT;
	DrawNodes(true);
...
void Scene::BuildScreenFBO()
{
	...
	const int multisample_level = 4;
	unsigned int tex_ref = GL_TEXTURE_2D;

	if (multisample_level == 0)
	{
		//Generate our Scene Depth Texture
		glBindTexture(GL_TEXTURE_2D, m_ScreenDTex);
	...
	{
		tex_ref = GL_TEXTURE_2D_MULTISAMPLE;

		//Generate our Scene Depth Texture
		glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, m_ScreenDTex);
		glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, multisample_level, GL_DEPTH24_STENCIL8, width, height, GL_FALSE);

		//Generate our Scene Colour Texture	
		glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, m_ScreenCTex);
		glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, multisample_level, GL_RGBA8, width, height, GL_FALSE);
	}
	...
	glBindFramebuffer(GL_FRAMEBUFFER, m_ScreenFBO);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, tex_ref, m_ScreenDTex, 0);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, tex_ref, m_ScreenDTex, 0);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, tex_ref, m_ScreenCTex, 0);
