...
void FEM3NodedFast::simulation_OnClothDesignChanged(ClothDesignEntity<3>* design)
{
	...
	m_PhyxelsTriangleLookupIJ.resize(m_NumTotal);
	...
	for (int i = 0; i < (int)m_NumTotal; ++i)
	{
		...
		const auto& tris = m_PhyxelsTriangleLookupIJ[i];
...
void  FEM3NodedFast::UpdateConstraints()
{
...
void FEM3NodedFast::Simulation_StepSimulation(float dt)
{
...
#pragma omp parallel for
		...
		for (uint i = 0; i < m_NumTotal; ++i)
		{
			if (m_PhyxelIsStatic[i])
			{
				float spd = (i < (m_NumTotal >> 1)) ? 1.85f : 0.0f;
...
bool FEM3NodedFast::ValidateVelocityTimestep()
{
...
void FEM3NodedFast::CollideEllipsoid(const Ellipsoid& e)
{
#pragma omp parallel for
	for (int i = 0; i < (int)m_NumTotal; i++) {
		if (m_PhyxelIsStatic[i])
...
void FEM3NodedFast::GenFEMTriangle(unsigned int idx, FETriangle& tri)
{
...
Matrix3 FEM3NodedFast::BuildStiffnessMatrix(const Vector3& dNa, const Vector3& dNb, float triArea)
{
	...
	//Ke = Ke * 0.5f;
	float lambda = 100.f;// 1366.f;
	float mu = 100.f;// 800.f;
	...
	*/
	...
	//Ke = (Ke + Matrix3::TransposeMirror(Ke)) * 0.5f;
...
void FEM3NodedFast::BuildRotationAndNormals()
{
...
Matrix3 FEM3NodedFast::BuildRotationMatrix(const Vector3& a, const Vector3& b, const Vector3& c)
{
	Vector3 t_rows[3];
	t_rows[0] = a - c;
	t_rows[1] = b - c;
...
void FEM3NodedFast::SimpleCorotatedBuildAMatrix(float dt)
{
#define LOCK_FREE_AMATRIX 1


	const float dt2 = dt * dt;
	const float dt2_visos = dt2 + V_SCALAR * dt;

#if !LOCK_FREE_AMATRIX
#pragma omp parallel for
	for (int i = 0; i < (int)m_NumTotal; ++i)
	{
		m_Solver.AMatrix(i, i) = Matrix3::Identity * m_PhyxelsMass[i];
		m_Solver.BVector(i) = m_PhyxelsVel[i] * m_PhyxelsMass[i] + m_PhyxelForces[i] * dt;
	}
#endif
...
#if LOCK_FREE_AMATRIX
		int i3 = i * 3;
		Vector3* B_i = &m_TrianglesB[i3];
		Matrix3* A_ii = &m_TrianglesAii[i3];
		Matrix3* A_ij = &m_TrianglesAij[i3];
#else
		Vector3 B_i[3];
		Matrix3 A_ii[3];
		Matrix3 A_ij[3];
...
#pragma omp critical(updateAB)
		{
			for (uint j = 0; j < 3; ++j)
			{
				...
				m_Solver.AMatrix(idxA, idxA) += A_ii[j];
				m_Solver.AMatrix(idxA, idxB) += A_ij[j];
				m_Solver.AMatrix(idxB, idxA) += A_ij[j];

				m_Solver.BVector(idxA) += B_i[j] * dt;
			}
		}
#endif
	}
...
#if USE_DYNAMIC_MINMAX
	m_RenderValueMinMax.x = FLT_MAX;
	m_RenderValueMinMax.y = -FLT_MAX;

	float minval, maxval;
#pragma omp parallel private(minval, maxval)
	{
		minval = FLT_MAX;
		maxval = -FLT_MAX;
...
#pragma omp parallel for
		...
		for (int i = 0; i < (int)m_NumTotal; ++i)
		{
			...
			{
				BVec += m_TrianglesB[tris[itr]];
				Aii += m_TrianglesAii[tris[itr]];
			}
...
#if USE_DYNAMIC_MINMAX		
			minval = min(minval, m_RenderValue[i]);
			maxval = max(maxval, m_RenderValue[i]);
#endif
			BVec += m_PhyxelForces[i];
			BVec *= dt;
			BVec += m_PhyxelsVel[i] * mass;
			...
			uint idxA = i;
			const auto& edges = m_PhyxelsTriangleLookupIJ[i];
			for (auto itr = edges.begin(), end = edges.end(); itr != end; ++itr)
			{
				...
				{
					Aij += m_TrianglesAij[itr->second[k]];
				}
		...
		if (minval < m_RenderValueMinMax.x) {
			#pragma critical
			{
				if (minval < m_RenderValueMinMax.x) m_RenderValueMinMax.x = minval;
			}
		}

		if (maxval > m_RenderValueMinMax.y) {
			#pragma critical
			{
				if (maxval > m_RenderValueMinMax.y) m_RenderValueMinMax.y = maxval;
			}
		}
#endif
	}
...
void FEM3NodedFast::Render_DrawingToVisualiser()
{
	unsigned int i;
...
void FEM3NodedFast::BuildTriangleMappings()
{
	for (unsigned int i = 0; i < m_NumTriangles; ++i)
	{
		unsigned int i3 = i * 3;
		...
		//AB
		m_PhyxelsTriangleLookupIJ[idxA][idxB].push_back(i3 + 0);
		m_PhyxelsTriangleLookupIJ[idxB][idxA].push_back(i3 + 0);

		//BC
		m_PhyxelsTriangleLookupIJ[idxB][idxC].push_back(i3 + 1);
		m_PhyxelsTriangleLookupIJ[idxC][idxB].push_back(i3 + 1);

		//CA
		m_PhyxelsTriangleLookupIJ[idxC][idxA].push_back(i3 + 2);
		m_PhyxelsTriangleLookupIJ[idxA][idxC].push_back(i3 + 2);
	}
}