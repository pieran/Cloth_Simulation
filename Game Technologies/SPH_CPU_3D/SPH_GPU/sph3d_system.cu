...
uint iDivUp(uint i, uint j)
{
	return (uint)(ceil(float(i) / float(j)));
}
...
__global__
void copy_buffer_kernel(Particle3D *dMem, float4 *buffer, uint num_particle)
{
...
void copy_buffer(Particle3D *dMem, float4 *buffer, uint num_particle)
{
	if(num_particle == 0)
	{
		return;
...
__global__
void calc_hashK(uint *dHash, uint *dIndex, Particle3D *dMem, uint num_particle)
{
    uint index=blockIdx.x*blockDim.x+threadIdx.x;
...
void calc_hash(uint *dHash, uint *dIndex, Particle3D *dMem, uint num_particle)
{
	if(num_particle == 0)
	{
		return;
...
__global__
void integrate_velocity_kernel(Particle3D* dMem, uint num_particle)
{
	uint index=blockIdx.x*blockDim.x+threadIdx.x;
...
void integrate_velocity(Particle3D *dMem, uint num_particle)
{
	if(num_particle == 0)
	{
		return;
...
__device__
float compute_cell_density(uint index, int3 neighbor, Particle3D *dMem, uint *dHash, uint *dIndex, uint *dStart, uint *dEnd, uint num_particle, uint tot_cell)
{
...
__global__
void compute_density_kernel(Particle3D *dMem, uint *dHash, uint *dIndex, uint *dStart, uint *dEnd, uint num_particle, uint tot_cell)
{
	uint index=blockIdx.x*blockDim.x+threadIdx.x;
...
__device__
float3 compute_cell_force(uint index, int3 neighbor, Particle3D *dMem, uint *dHash, uint *dIndex, uint *dStart, uint *dEnd, uint num_particle, uint tot_cell, float3 &grad_color, float &lplc_color)
{
...
__global__
void compute_force_kernel(Particle3D *dMem, uint *dHash, uint *dIndex, uint *dStart, uint *dEnd, uint num_particle, uint tot_cell)
{
	uint index=blockIdx.x*blockDim.x+threadIdx.x;
...
void compute(Particle3D *dMem, uint *dHash, uint *dIndex, uint *dStart, uint *dEnd, uint num_particle, uint tot_cell)
{
	if(num_particle == 0)
	{
		return;
