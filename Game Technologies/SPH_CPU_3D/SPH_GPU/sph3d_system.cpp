...
SPH3DSystem::SPH3DSystem()
{
	...
	hMem=(Particle3D *)malloc(sizeof(Particle3D)*hParam->max_particle);
	alloc_array((void**)&(dMem), sizeof(Particle3D)*hParam->max_particle);
...
{	
	...
	copy_array(hMem, dMem, sizeof(Particle3D)*hParam->num_particle, CUDA_DEV_TO_HOST);
}

void SPH3DSystem::add_particle(float3 pos, float3 vel)
{
	Particle3D *p=&(hMem[hParam->num_particle]);
...
void SPH3DSystem::init_system()
{
	...
	copy_array(dMem, hMem, sizeof(Particle3D)*hParam->num_particle, CUDA_HOST_TO_DEV);
	printf("Init Particle: %u\n", hParam->num_particle);
}

void SPH3DSystem::set_sim(float3 ratio, float3 origin)
{
