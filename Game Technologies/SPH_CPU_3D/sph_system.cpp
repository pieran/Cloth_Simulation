...
void SPHSystem::init_system()
{
	...
	{
		for (pos.y = world_size.y*0.0f; pos.y<world_size.y*0.6f; pos.y += (kernel*0.5f))
		{
...
void SPHSystem::add_particle(Vector3 pos, Vector3 vel)
{
...
#pragma omp parallel for private(p, np, cell_pos, near_pos, hash, rel_pos, r2)
	for (int i = 0; i<(int)num_particle; i++)
	{
			...
			for (int y = -1; y <= 1; y++)
			{
				for (int z = -1; z <= 1; z++)
				{
					...
					while (np != NULL)
					{
						...
						p->dens = p->dens + mass * poly6_value * POW3(kernel_2 - r2);
						np = np->next;
					}
				}
			}
		...
		p->pres = (POW7(p->dens / rest_density) - 1) *gas_constant;
	}
}
...
#pragma omp parallel for private(p, np, cell_pos, near_pos, hash, rel_pos, rel_vel, r2, r, kernel_r, V, pres_kernel, visc_kernel, temp_force, grad_color, lplc_color)
	for (int i = 0; i<(int)num_particle; i++)
	{
		p = &(mem[i]);
		cell_pos = calc_cell_pos(p->pos);
...
int3 SPHSystem::calc_cell_pos(Vector3 p)
{
	int3 cell_pos;
